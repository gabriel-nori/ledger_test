...
class PersonTest(TestCase):
    ...
    def test_age(self):
        ...
        assert self.test_objects.underage_client.birthday >= date.today()-timedelta(days=16*365)
        assert self.test_objects.michael_ok.birthday <= date.today()-timedelta(days=16*365)
    ...
    def test_age_validators(self):
        ...
        """
        assert not personUtils.check_16_older(self.test_objects.underage_client.birthday)
        assert not personUtils.check_18_older(self.test_objects.underage_client.birthday)
